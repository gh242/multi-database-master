    ...
    public partial class Sys_TableInfoService
    {
        ...
        private string WebProject
        {
            get
            {
                ...
                if (webProject == null)
                {
                    throw new Exception("未獲取到以.WebApi結尾的项目名称,无法创建页面");
                }
                return webProject;
            }
        }
        private string ApiNameSpace
        {
            get
            {
                ...
                if (apiNameSpace == null)
                {
                    throw new Exception("未獲取到以.WebApi,无法创建Api控制器");
                }
                return apiNameSpace;
            }
        }
        ...
        /// <summary>
        /// 2020.05.17增加mysql獲取表結构时区分當前所在數據庫
        /// </summary>
        /// <returns></returns>
        /// <returns></returns>
        private string GetMysqlTableSchema(string dbConnection)
        {
            ...
            {
                Console.WriteLine($"獲取mysql數據庫名异常:{ex.Message}");
                return "";
            }
        }
        ...
        /// <summary>
        /// 獲取Mysql表結构信息
        /// 2020.06.14增加对mysql数据類型double区分
        /// </summary>
        /// <returns></returns>
...
DISTINCT
	...
	SELECT CASE WHEN t.ColumnType IN ('DECIMAL','smallmoney','money') THEN 
        ...
        private WebResponseContent ExistsTable(string tableName, string tableTrueName)
        {
            ...
            //如果是第一次创建model，此處反射獲取到的是已經緩存過的文件，必須重新运行项目否则新增的文件无法做判断文件是否创建，需要重新做反射实际文件，待修改...
                ...
                .GetTypes().Where(x => x.GetTypeInfo().BaseType != null
                    && x.BaseType == typeof(BaseEntity)))
                    {
                        ...
                        if (entity.Name != tableName)
                        {
                            ...
                            {
                                return webResponse.Error($"实际表名【{tableTrueName }】已被【{entity.Name}】创建建实体,不能创建别名【{tableName}】实体,請將别名更换為【{entity.Name}】");
                            }
                        }
                    }
                }
        ...
        private ISqlDapper GetDapper(string dbServer)
        {
            ...
            switch (dbServer)
            {
                ...
                case "ReportDbContext":
                    ...
                    break;
                case "EDCDbContext":
                    sqlDapper = DBServerProvider.SqlDapperEDC;
                    break;
                default:
        ...
        public WebResponseContent SaveEidt(Sys_TableInfo sysTableInfo)
        {
            ...
            {
                return WebResponseContent.Instance.Error($"不能將字段【{sysTableInfo.ExpressField}】設置為快捷编辑,因為已經設置了数据源");
            }
        ...
        public async Task<WebResponseContent> SyncTable(string tableName)
        {
          ...
          .Include(o => o.TableColumns).FirstOrDefault();
            ...
            if (columns == null || columns.Count == 0)
                return webResponse.Error("未獲取到【" + tableName + "】表結构信息，請确认表是否存在");
            ...
            {
                return webResponse.Error("【" + tableName + "】表結构未发生變化");
            }
        ...
        /// <summary>
        /// 生成Services/Repository與对應的Partial類
        /// </summary>
        /// <param name="tableName"></param>
        ...
        public string CreateServices(string tableName, string nameSpace, string foldername, bool webController, bool apiController)
        {
            ...
            if (apiController)
            {
                ...
                {
                    return "未找到webapi類庫,請确认是存在weiapi類庫命名以.webapi結尾";
                }
        ...
        /// </summary>
        /// <param name="sysTableInfo"></param>
        /// <param name="vuePath">為本地Vue项目Views所在的绝对路徑:E:/web/myProject/Views</param>
        /// <returns></returns>
        public string CreateVuePage(Sys_TableInfo sysTableInfo, string vuePath)
        {
            ...
            {
                return isApp ? "請設置App路徑" : "請設置Vue所在Views的绝对路徑!";
            }

            if (!FileHelper.DirectoryExists(vuePath)) return $"未找项目路徑{vuePath}!";
            ...
            if (!isApp)
            {
                ...
                {
                    //2021.03.06增加前端生成文件到指定文件夹(以前生成過的文件不受影响)
        ...
        private string GetConnection(string dbServer)
        {
            ...
            switch (dbServer)
            {
                ...
                case "ReportDbContext":
                    ...
                    break;
                case "EDCtDbContext":
                    sqlDapper = DBServerProvider.EDCConnectingString;
                    break;
                default:
        ...
        public async Task<WebResponseContent> DelTree(int table_Id)
        {
            ...
            {
                return new WebResponseContent().Error("當前删除的节点存在表結构信息,只能删除空节点");
            }
            if (repository.Exists(x => x.ParentId == table_Id))
            {
                return new WebResponseContent().Error("當前删除的节点存在子节点，不能删除");
            }
        ...
        /// <param name="tableInfo"></param>
        /// <param name="createType">1、创建实体類,2创建apiinput類,3、创建apioutput類</param>
        private string CreateEntityModel(List<Sys_TableColumn> sysColumn, Sys_TableInfo tableInfo, List<TableColumnInfo> tableColumnInfoList, int createType)
        {
            ...
            foreach (Sys_TableColumn column in sysColumn)
            {
                ...
                //[Column(TypeName="bigint")]如果與字段類型不同会产生异常
                ...
                //如果主键是string,则默認為是Guid或者使用的是mysql数据，字段類型是字符串并且长度是36则默認為是Guid
                if ((column.IsKey == 1
            ...
            {
                //如果使用的是pgsql數據庫，并且數據庫表都是小写，請將下面的.ToLower()这段开启.2020.08.07
            ...
            if (!string.IsNullOrEmpty(tableInfo.TableTrueName) && tableInfo.TableName != tableInfo.TableTrueName)
            {
                ...
                //2020.06.14 pgsql數據庫，設置表名為小写(數據庫创建表的时候也要使用小写)
            ...
            if (!FileHelper.FileExists(modelPath + tableName + ".cs"))
            {
                partialContent = partialContent.Replace("{AttributeManager}", "")
                    .Replace("{AttributeList}", @"//此處配置字段(字段配置见此model的另一个partial),如果表中没有此字段請加上 [NotMapped]屬性，否则会异常")
                    .Replace(":{Temp_Entity}", "")
        ...
        private WebResponseContent ValidColumnString(Sys_TableInfo tableInfo)
        {
            ...
            if (!string.IsNullOrEmpty(tableInfo.DetailName))
            {
                ...
                if (mainTableColumn == null)
                    return webResponse.Error($"請勾选表[{tableInfo.TableName}]的主键");
                ...
                if (tableColumn == null)
                    return webResponse.Error($"明细表必須包括[{tableInfo.TableName}]主键字段[{key}]");

                if (mainTableColumn.ColumnType?.ToLower() != tableColumn.ColumnType?.ToLower())
                {
                    return webResponse.Error($"明细表的字段[{tableColumn.ColumnName}]類型必須與主表的主键的類型相同");
                }
                ...
                {
                    return webResponse.Error($"主表主键類型為Guid，明细表[{tableInfo.DetailName}]配置的字段[{key}]长度必須是36，請重將明细表字段[{key}]长度設置為36，点击保存與生成Model");
                }
            ...
            //{
            //    webResponse.Error("数据類型為string的列，必須输入[列最大长度]的值");
            //}
            return webResponse;
        }
