...
namespace VOL.System.Services
{
    public partial class Sys_UserService
    {
        ...
        public async Task<WebResponseContent> ModifyPwd(string oldPwd, string newPwd)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(oldPwd)) return webResponse.Error("旧密码不能為空");
                if (string.IsNullOrEmpty(newPwd)) return webResponse.Error("新密码不能為空");
                ...
                if (userCurrentPwd == _newPwd) return webResponse.Error("新密码不能與旧密码相同");
            ...
            {
                message = ex.Message;
                webResponse.Error("服务器了点問题,請稍后再试");
            }
            finally
            {
                if (message == "")
        ...
        public override WebResponseContent Add(SaveModel saveModel)
        {
            ...
            base.AddOnExecuted = (Sys_User user, object list) =>
            {
                ...
                return webResponse.OK($"用戶新建成功.帐号{user.UserName}密码{pwd}");
            };
        ...
        public override WebResponseContent Del(object[] keys, bool delList = false)
        {
            base.DelOnExecuting = (object[] ids) =>
            {
                if (!UserContext.Current.IsSuperAdmin)
                {
                    ...
                    {
                        return webResponse.Error($"没有权限删除用戶：{string.Join(',', userNames)}");
                    }
                }
        ...
        public override WebResponseContent Update(SaveModel saveModel)
        {
            ...
            base.UpdateOnExecuting = (Sys_User user, object obj1, object obj2, List<object> list) =>
            {
                ...
                //Sys_User实体的UserPwd用戶密码字段的屬性不是编辑，此處不会修改密码。但防止代码生成器將密码字段的修改成了可编辑造成密码被修改
        ...
        public void SaveDepartment(Guid[] deptIds, int userId)
        {
            ...
            //如果需要判断當前角色是否越权，再調用一下獲取當前部门下的所有子角色判断即可
