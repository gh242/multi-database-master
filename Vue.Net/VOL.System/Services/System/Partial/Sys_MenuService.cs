...
namespace VOL.System.Services
{
    ...
    {
        /// <summary>
        /// 菜单静态化處理，每次獲取菜单时先比较菜单是否发生變化，如果发生變化从數據庫重新獲取，否则直接獲取_menus菜单
        /// </summary>
        ...
        public async Task<object> GetMenuActionList(int roleId)
        {
            ...
            if (UserContext.IsRoleIdSuperAdmin(roleId))
            {
                ...
                new
                {
                    ...
                    icon = x.Icon,
                    x.Enable,
                    x.TableName, // 2022.03.26增移动端加菜单類型
            ...
            var menu = from a in UserContext.Current.Permissions
                       ...
                       select new
                       {
                           ...
                           icon = b.Icon,
                           b.Enable,
                           b.TableName, // 2022.03.26增移动端加菜单類型
        ...
        public async Task<WebResponseContent> Save(Sys_Menu menu)
        {
            ...
            if (menu == null) return webResponse.Error("没有獲取到提交的参数");
            if (menu.Menu_Id > 0 && menu.Menu_Id == menu.ParentId) return webResponse.Error("父级ID不能是當前菜单的ID");
            try
            {
                ...
                if (menu.TableName != "/" && menu.TableName != ".")
                {
                    ...
                    if (sysMenu != null)
                    {
                        ...
                        if (sysMenu.MenuType == menu.MenuType)
                        {
                            ...
                            || menu.Menu_Id <= 0)
                            {
                                return webResponse.Error($"視圖/表名【{menu.TableName}】已被其他菜单使用");
                            }
                        }
                    }
                }
                ...
                else
                {
                    ...
                    {
                        return webResponse.Error($"不能选择此父级id，选择的父级id與當前菜单形成依赖关系");
                    }
                    ...
                    repository.Update(menu.SetModifyDefaultVal(), p => new
                    {
                        ...
                        p.Icon,
                        p.Enable,
                        p.MenuType,// 2022.03.26增移动端加菜单類型
                        p.TableName,
                        p.ModifyDate,
                        p.Modifier
                    });
        ...
        public async Task<WebResponseContent> DelMenu(int menuId)
        {
            ...
            {
                return webResponse.Error("當前菜单存在子菜单,請先删除子菜单!");
            }
