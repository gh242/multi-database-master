...
namespace VOL.Core.Extensions
{
    public static class EntityProperties
    {
        ...
        /// 
        /// </summary>
        /// <param name="array">將数組转换成sql语句</param>
        ...
        /// <summary>
        ///<param name="sql">要執行的sql语句如：通過EntityToSqlTempName.Temp_Insert0.ToString()字符串占位，生成的的sql语句会把EntityToSqlTempName.Temp_Insert0.ToString()替换成生成的sql临时表数据
        ...
        /// <param name="fieldType">指定生成的数組值的類型</param>
        /// <param name="sql"></param>
        /// <returns></returns>
                  ...
                  )
        {
            ...
            {
                throw new Exception("写入数据太多,請分开写入。");
            }
            ...
            {
                //每1000行需要分批写入(數據庫限制每批至多写入1000行数据)
        ...
        /// <summary>
        ///此方法适用于数据量少,只有几列数据，不超過1W行，或几十列数据不超過1000行的情况下使用
        /// 大批量的数据考虑其他方式
        ...
        /// <param name="keyType">true獲取key对應類型,false返回对象Key的名称</param>
        /// <returns></returns>
        public static string GetKeyName(this PropertyInfo[] properties, bool keyType)
        {
            ...
            foreach (PropertyInfo propertyInfo in properties)
            {
                ...
                //如果没有ColumnAttribute的需要单独再验证，下面只验证有屬性的
        ...
        /// <param name="expression">对指定屬性进行验证x=>{x.Name,x.Size}</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// specificProperties=null并且validateProperties=null，对所有屬性验证，只验证其是否合法，不验证是否為空(除屬性标识指定了不能為空外)
        /// specificProperties!=null，对指定屬性校验，并且都必須有值
        /// null并且validateProperties!=null，对指定屬性校验，不判断是否有值
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entity"></param>
        /// <param name="specificProperties">验证指定的屬性，并且非空判断</param>
        /// <param name="validateProperties">验证指定屬性，只对字段合法性判断，不验证是否為空</param>
        /// <returns></returns>
        ...
        {
            WebResponseContent responseData = new WebResponseContent();
            if (entity == null) return responseData.Error("对象不能為null");
            ...
            foreach (PropertyInfo propertyInfo in propertyArray)
            {
                ...
                //若存在specificProperties并且屬性為数組specificProperties中的值，校验时就需要判断是否為空
        ...
        /// <summary>
        /// 獲取數據庫類型，不带长度，如varchar(100),只返回的varchar
        /// </summary>
        /// <param name="propertyInfo"></param>
        /// <returns></returns>
        ...
        /// 如字段:标识為數據庫int類型[Column(TypeName="int")]  public int Id { get; set; }
        /// 如果是小数float或Decimal必須对propertyInfo字段加DisplayFormatAttribute屬性
        /// </param>
        /// <param name="value"></param>
        /// <returns>IEnumerable<(bool, string, object)> bool成否校验成功,string校验失败信息,object,當前校验的值</returns>
        ...
        /// 验证數據庫字段類型與值是否正确，
        /// </summary>
        /// <param name="dbType">數據庫字段類型(如varchar,nvarchar,decimal,不要带后面长度如:varchar(50))</param>
        /// <param name="value">值</param>
        /// <param name="propertyInfo">要验证的類的屬性，若不為null，则会判断字符串的长度是否正确</param>
        /// <returns>(bool, string, object)bool成否校验成功,string校验失败信息,object,當前校验的值</returns>
        public static (bool, string, object) ValidationVal(this string dbType, object value, PropertyInfo propertyInfo = null)
        {
            ...
            else if (dbType == SqlDbTypeName.Float || dbType == SqlDbTypeName.Decimal || dbType == SqlDbTypeName.Double)
            {
                ...
                //    throw new Exception("請对字段" + propertyInfo?.Name + "添加DisplayFormat屬性标识");
                ...
                || dbType == SqlDbTypeName.Text))
            {
                ...
                //默認nvarchar(max) 、text 长度不能超過20000
        ...
        public static (bool, string, object) ValidationProperty(this PropertyInfo propertyInfo, object objectVal, bool required)
        {
            ...
            if ((required && val == string.Empty))
            {
                ...
                return (false, requiredMsg + (string.IsNullOrEmpty(propertyName) ? propertyInfo.Name : propertyName) + "不能為空", objectVal);
            }
            //列名
        ...
        private static string ValidateDicInEntity(this Type typeinfo, Dictionary<string, object> dic, PropertyInfo[] propertyInfo, bool removeNotContains, bool removerKey, string[] ignoreFields = null)
        {
            ...
            foreach (PropertyInfo property in propertyInfo)
            {
                ...
                {
                    //移除主键默認為新增数据，將不在编辑列中的有默認值的数据設置為默認值
                    ...
                    {
                        return property.GetTypeCustomValue<DisplayAttribute>(x => x.Name) + "為必須提交项";
                    }
                    continue;
                }
                ...
                //removerKey=false,保留主键，屬性與主键不同的直接移除
                //  if (!isEdit && (removerKey || (!removerKey && property.Name != keyName)))
                if (!isEdit)
                {
                    if (property.GetCustomAttributes(typeof(RequiredAttribute)).Count() > 0)
                    {
                        return property.GetTypeCustomValue<DisplayAttribute>(x => x.Name) + "没有配置好Model為编辑列";
                    }
        ...
        /// <summary>
        /// 將实体映射到输出到ApiOutput实体上,如果没有输入实体，则返回null
        /// </summary>
        /// <param name="entity"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 將数据源映射到新的数据中,目前只支持List<TSource>映射到List<TResult>或TSource映射到TResult
        /// 目前只支持Dictionary或实体類型
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <typeparam name="TResult"></typeparam>
        /// <param name="source"></param>
        /// <param name="resultExpression">只映射返回对象的指定字段,若為null则默認為全部字段</param>
        /// <param name="sourceExpression">只映射数据源对象的指定字段,若為null则默認為全部字段</param>
        /// <returns></returns>
        ...
        /// <param name="expression">指定对需要的字段赋值,格式x=>new {x.Name,x.P},返回的結果只会对Name與P赋值</param>
        ...
        /// <summary>
        /// 設置默認字段的值"CreateID", "Creator", "CreateDate"，"ModifyID", "Modifier", "ModifyDate"
        /// </summary>
        /// <param name="saveDataModel"></param>
        /// <param name="setType">true=新增設置"CreateID", "Creator", "CreateDate"值
        /// false=编辑設置"ModifyID", "Modifier", "ModifyDate"值
        /// </param>
        ...
        /// <summary>
        /// 
        /// 設置默認字段的值如:"CreateID", "Creator", "CreateDate"，"ModifyID", "Modifier", "ModifyDate"
        /// </summary>
        /// <param name="saveDataModel"></param>
        /// <param name="setType">true=新增設置"CreateID", "Creator", "CreateDate"值
        /// false=编辑設置"ModifyID", "Modifier", "ModifyDate"值
