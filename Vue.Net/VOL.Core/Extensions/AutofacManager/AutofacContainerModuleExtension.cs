    ...
    public static class AutofacContainerModuleExtension
    {
        ...
        public static IServiceCollection AddModule(this IServiceCollection services, ContainerBuilder builder, IConfiguration configuration)
        {
            ...
            if (DBType.Name == DbCurrentType.MySql.ToString())
            {
                ...
                //mysql8.x的版本使用Pomelo.EntityFrameworkCore.MySql 3.1会产生异常，需要在字符串連接上添加allowPublicKeyRetrieval=true
                services.AddDbContextPool<BaseDbContext>(optionsBuilder => { optionsBuilder.UseMySql(connectionString); }, 64);
            }
            else if (DBType.Name == DbCurrentType.PgSql.ToString())
            {
                services.AddDbContextPool<BaseDbContext>(optionsBuilder => { optionsBuilder.UseNpgsql(connectionString); }, 64);
            }
            else
            {
                services.AddDbContextPool<BaseDbContext>(optionsBuilder => { optionsBuilder.UseSqlServer(connectionString); }, 64);
            }
            //启用緩存
