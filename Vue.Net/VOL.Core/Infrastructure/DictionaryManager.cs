...
namespace VOL.Core.Infrastructure
{
    public static class DictionaryManager
    {
        ...
        /// <param name="executeSql">是否執行自定义sql</param>
        /// <returns></returns>
        public static IEnumerable<Sys_Dictionary> GetDictionaries(IEnumerable<string> dicNos, bool executeSql = true)
        {
            static List<Sys_DictionaryList> query(string sql, string dbServer)
            {
                try
                {
                    return DBServerProvider.GetSqlDapperWithName(DBServerProvider.GetContextName(dbServer)).QueryList<SourceKeyVaule>(sql, null).Select(s => new Sys_DictionaryList()
                    {
                        DicName = s.Value,
                ...
                {
                    Logger.Error($"字典執行sql异常,sql:{sql},异常信息：{ex.Message + ex.StackTrace}");
                    throw ex;
        ...
        /// <summary>
        /// 每次變更字典配置的时候会重新拉取所有配置进行緩存(自行根据实际處理)
        /// </summary>
        /// <returns></returns>
