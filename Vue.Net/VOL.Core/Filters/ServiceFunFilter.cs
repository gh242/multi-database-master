...
namespace VOL.Core.Filters
{
    public abstract class ServiceFunFilter<T> where T : class
    {
        ...
        /// <summary>
        /// 是否开启用戶数据权限,true=用戶只能操作自己(及下级角色)创建的数据,如:查询、删除、修改等操作
        ...
        ///  select * 這裡可以自定义，但select 必須返回表所有的列，不能少
        /// </summary>
        ...
        /// <summary>
        /// 調用新建處理前(SaveModel為传入的原生数据)
        /// </summary>
        protected Func<SaveModel, WebResponseContent> AddOnExecute;

        /// <summary>
        /// 調用新建保存數據庫前處理(已將提交的原生数据转换成了对象)
        ///  Func<T, object,ResponseData>  T為主表数据，object為明细数据(如果需要使用明细对象,請用 object as List<T>转换) 
        /// </summary>
        protected Func<T, object, WebResponseContent> AddOnExecuting;

        /// <summary>
        /// 調用新建保存數據庫后處理。
        /// **实现當前方法时，内部默認已經开启事务，如果实现的方法操作的是同一數據庫,则不需要在AddOnExecuted中事务
        ///  Func<T, object,ResponseData>  T為主表数据，object為明细数据(如果需要使用明细对象,請用 object as List<T>转换) 
        ///  此處已开启了DbContext事务(重点),如果还有其他業務處事，直接在這裡写EF代码,不需要再开启事务
        /// 如果執行的是手写sql請用repository.DbContext.Database.ExecuteSqlCommand()或 repository.DbContext.Set<T>().FromSql執行具体sql语句
        /// </summary>
        ...
        /// <summary>
        /// 調用更新方法前處理(SaveModel為传入的原生数据)
        /// </summary>
        protected Func<SaveModel, WebResponseContent> UpdateOnExecute;

        /// <summary>
        ///  調用更新方法保存數據庫前處理
        ///  (已將提交的原生数据转换成了对象,將明细新增、修改、删除的数据分别用object1/2/3标识出来 )
        ///  T=更新的主表对象
        ///  object1=為新增明细的对象，使用时將object as List<T>转换一下
        ///  object2=為更新明细的对象
        ///  List<object>=為删除的细的对象Key
        /// </summary>
        protected Func<T, object, object, List<object>, WebResponseContent> UpdateOnExecuting;

        /// <summary>
        ///  調用更新方法保存數據庫后處理
        ///   **实现當前方法时，内部默認已經开启事务，如果实现的方法操作的是同一數據庫,则不需要在UpdateOnExecuted中事务
        ///  (已將提交的原生数据转换成了对象,將明细新增、修改、删除的数据分别用object1/2/3标识出来 )
        ///  T=更新的主表对象
        ///  object1=為新增明细的对象，使用时將object as List<T>转换一下
        ///  object2=為更新明细的对象
        ...
        /// <summary>
        /// 删除前處理,object[]准备删除的主键
        /// </summary>
        protected Func<object[], WebResponseContent> DelOnExecuting;

        /// <summary>
        /// 删除后處理,object[]已删除的主键,此處已开启了DbContext事务(重点),如果还有其他業務處事，直接在這裡写EF代码,不需要再开启事务
        /// 如果執行的是手写sql請用repository.DbContext.Database.ExecuteSqlCommand()或 repository.DbContext.Set<T>().FromSql執行具体sql语句
        /// </summary>
        ...
        /// string=當前读取的excel单元格的值
        /// ExcelWorksheet=excel对象
        /// ExcelRange當前excel单元格对象
        /// int=當前读取的第几数
        /// int=當前读取的第几列
        /// string=返回的值
        /// </summary>
