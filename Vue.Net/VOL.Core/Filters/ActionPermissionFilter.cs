...
namespace VOL.Core.Filters
{
    ...
    /// 2、TableName、TableAction 同时為null，SysController為false的，只判断是否登陆
    /// 3、TableName、TableAction 都不null根据表名與action判断是否有权限
    /// 4、SysController為true，通過httpcontext獲取表名與action判断是否有权限
    /// 5、Roles对指定角色验证
    /// </summary>
    public class ActionPermissionFilter : IAsyncActionFilter
    {
        ...
        private WebResponseContent OnActionExecutionPermission(ActionExecutingContext context)
        {
            ...
            //如果没有指定表的权限，则默認為代码生成的控制器，优先獲取PermissionTableAttribute指定的表，如果没有数据则使用當前控制器的名作為表名权限
            ...
            //2020.05.05移除x.TableName.ToLower()转换,獲取权限时已經转换成為小写
