...
{
    /// <summary>
    /// 消费者 (Message.Key的数据類型為string、Message.Value的数据類型為string）
    /// 消费者实现三种消费方式：1.订阅回調模式 2.批量消费模式 3.单笔消费模式
    /// </summary>
    /// <typeparam name="TKey">Message.Key 的数据類型</typeparam>
    /// <typeparam name="TValue">Message.Value 的数据類型</typeparam>
                ...
                while (AppSetting.Kafka.IsConsumerSubscribe) //true
                {
                    ...
                    try
                    {
                        ...
                        if (Func(result))
                        {
                            ...
                            {
                                //手动提交，如果上面的EnableAutoCommit=true表示自动提交，则无需調用Commit方法
