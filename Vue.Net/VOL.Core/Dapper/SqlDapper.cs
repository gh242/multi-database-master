...
namespace VOL.Core.Dapper
{
    public class SqlDapper : ISqlDapper
    {
        ...
        public int AddRange<T>(IEnumerable<T> entities, Expression<Func<T, object>> addFileds = null, bool beginTransaction = true)
        {
            ...
            if (key == null)
            {
                throw new Exception("实体必須包括主键才能批量更新");
            }
            string[] columns;
            ...
            else
            {
                ...
                //2020.11.21修复sqlserver批量写入主键類型判断错误
        ...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entity">实体必須带主键</param>
        ...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entities">实体必須带主键</param>
        ...
        private int MySqlBulkInsert(DataTable table, string tableName, string fileName = null, string tmpPath = null)
        {
            ...
            string text = $"當前行:{table.Rows.Count}";
        ...
        {
            //以半角逗号（即,）作分隔符，列為空也要表达其存在。
            //列内容如存在半角逗号（即,）则用半角引号（即""）將该字段值包含起来。
            //列内容如存在半角引号（即"）则應替换成半角双引号（""）转义，并用半角引号（即""）將该字段值包含起来。
            ...
            foreach (DataRow row in table.Rows)
            {
                for (int i = 0; i < table.Columns.Count; i++)
                {
                    ...
                    {
                        //centos系統里把datatable里的日期转换成了10/18/18 3:26:15 PM格式
