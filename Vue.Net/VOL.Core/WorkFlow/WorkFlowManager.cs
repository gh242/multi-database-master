...
namespace VOL.Core.WorkFlow
{
    public static class WorkFlowManager
    {
        ...
        public static void AddProcese<T>(T entity, bool rewrite = false, bool changeTableStatus = true, Action<T, List<int>> addWorkFlowExecuted = null) where T : class
        {
            ...
            //{
            //    if (setp.StepType == (int)AuditType.用戶审批)
            //    {
            ...
            ) where T : class
        {
            ...
            if (workFlow == null)
            {
                return webResponse.Error("未查到流程信息,請检查数据是否被删除");
            }
            ...
            if (currentStep == null)
            {
                return webResponse.Error($"未查到流程陈点[workFlow.CurrentStepId]信息,請检查数据是否被删除");
            }
            ...
            {
                if (status == AuditStatus.审核通過)
                {
                    workFlow.CurrentStepId = "审核完成";
                }
                else
                {
                    workFlow.CurrentStepId = "流程結束";
                }
                workFlow.AuditStatus = (int)status;
            ...
            //指向下一个人审批
            if (nextStep != null && status == AuditStatus.审核通過)
            {
        ...
        private static bool CheckAuditStatus(Sys_WorkFlowTable workFlow, FilterOptions filterOptions, Sys_WorkFlowTableStep currentStep, AuditStatus status)
        {
            //如果审核拒绝或驳回并退回上一步，待完
            //重新配置流程待完
            if (status != AuditStatus.审核未通過 && status != AuditStatus.驳回)
            {
                return true;
            }
