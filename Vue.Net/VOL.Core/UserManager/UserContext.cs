...
namespace VOL.Core.ManageUser
{
    ...
    {
        /// <summary>
        /// 為了尽量减少redis或Memory读取,保证執行效率,將UserContext注入到DI，
        /// 每个UserContext的屬性至多读取一次redis或Memory緩存从而提高查询效率
        /// </summary>
        ...
        /// <summary>
        /// 每个角色ID对應的菜单权限（已做静态化處理）
        /// 每次獲取权限时用當前服务器的版本号與redis/memory緩存的版本比较,如果不同会重新刷新緩存
        /// </summary>
        ...
        public List<Permissions> GetPermissions(int roleId)
        {
            if (IsRoleIdSuperAdmin(roleId))
            {
                ...
                var permissions = DBServerProvider.DbContext.Set<Sys_Menu>()
                    ...
                    .Select(a => new Permissions
                    {
                        Menu_Id = a.Menu_Id,
                        ParentId = a.ParentId,
                        //2020.05.06增加默認將表名转换成小写，权限验证时不再转换
            ...
            //角色有緩存，并且當前服务器的角色版本号與redis/memory緩存角色的版本号相同直接返回静态对象角色权限
            ...
            //锁定每个角色，通過安全字典减少锁粒度，否则多个同时角色獲取緩存会导致阻塞
            ...
            lock (objId)
            {
                ...
                //没有redis/memory緩存角色的版本号或與當前服务器的角色版本号不同时，刷新緩存
