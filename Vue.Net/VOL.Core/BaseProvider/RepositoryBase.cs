...
namespace VOL.Core.BaseProvider
{
    public abstract class RepositoryBase<TEntity> where TEntity : BaseEntity
    {
        ...
        /// <summary>
        /// 通過条件查询返回指定列的数据(將TEntity映射到匿名或实体T)
        ...
        /// <param name="orderBySelector">多个排序字段key為字段，value為升序/降序</param>
        /// <returns></returns>
            ...
            bool delNotExist = false) where TDetail : class
        {
            ...
            list.ForEach(x =>
            {
                ...
                {
                    //獲取所有修改的key,如果从數據庫查来的key,不在修改中的key，则為删除的数据
                    keys.Add(val);
        ...
        /// <summary>
        /// 注意List生成的table的列顺序必須要和數據庫表的列顺序一致
        /// </summary>
        ...
        /// <param name="value">查询的類</param>
        /// <param name="linqExpression">查询類型</param>
        /// <returns></returns>
