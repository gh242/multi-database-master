...
namespace VOL.Core.Quartz
{
    public static class QuartzNETExtension
    {
        ...
        public static IApplicationBuilder UseQuartz(this IApplicationBuilder applicationBuilder, IWebHostEnvironment env)
        {
            ...
            try
            {
                _taskList = services.GetService<BaseDbContext>().Set<Sys_QuartzOptions>().Where(x=>x.Status==0).ToList();
                for (int i = 0; i < 1; i++)
                {
        ...
        {
            return schedulerFactory.TriggerAction(JobAction.立即執行, taskOptions);
        }

        /// <summary>
        /// 触发新增、删除、修改、暂停、启用、立即執行事件
        /// </summary>
        /// <param name="schedulerFactory"></param>
        /// <param name="taskName"></param>
        /// <param name="groupName"></param>
        ...
        /// 
        /// </summary>
        /// <param name="context"></param>通過作业上下文獲取作业对應的配置参数
        /// <returns></returns>
        ...
        public static (bool, object) Exists(this Sys_QuartzOptions taskOptions, bool init)
        {
            if (!init && _taskList.Any(x => x.TaskName == taskOptions.TaskName && x.GroupName == taskOptions.GroupName))
            {
                return (false,
                    ...
                    {
                        status = false,
                        msg = $"作业:{taskOptions.TaskName},分組：{taskOptions.GroupName}已經存在"
                    });
            }
            return (true, null);
        }
        ...
        public static (bool, string) IsValidExpression(this string cronExpression)
        {
            try
            {
                ...
                return (date != null, date == null ? $"請确认表达式{cronExpression}是否正确!" : "");
            }
            catch (Exception e)
            {
                return (false, $"請确认表达式{cronExpression}是否正确!{e.Message}");
            }
        }
    }
