...
namespace VOL.Core.Utilities
{
    public class FileHelper
    {
        ...
        /// <summary>
        /// 通過迭代器读取平面文件行内容(必須是带有\r\n换行的文件,百万行以上的内容读取效率存在問题,适用于100M左右文件，行100W内，超出的会有卡顿)
        /// </summary>
        /// <param name="fullPath">文件全路徑</param>
        /// <param name="page">分页页数</param>
        /// <param name="pageSize">分页大小</param>
        /// <param name="seekEnd"> 是否最后一行向前读取,默認从前向后读取</param>
        /// <returns></returns>
        ...
        public static void CopyDir(string srcPath, string aimPath)
        {
            try
            {
                ...
                // 检查目标目錄是否以目錄分割字符結束如果不是则添加之
                if (aimPath[aimPath.Length - 1] != Path.DirectorySeparatorChar)
                    aimPath += Path.DirectorySeparatorChar;
                // 判断目标目錄是否存在如果不存在则新建之
                if (!Directory.Exists(aimPath))
                    Directory.CreateDirectory(aimPath);
                // 得到源目錄的文件列表，该里面是包含文件以及目錄路徑的一个数組
                //如果你指向copy目标文件下面的文件而不包含目錄請使用下面的方法
                //string[] fileList = Directory.GetFiles(srcPath);
                string[] fileList = Directory.GetFileSystemEntries(srcPath.ReplacePath());
                //遍历所有的文件和目錄
                foreach (string file in fileList)
                {
                    //先當作目錄處理如果存在这个目錄就递归Copy该目錄下面的文件
        ...
        /// 獲取文件夹大小
        /// </summary>
        /// <param name="dirPath">文件夹路徑</param>
        /// <returns></returns>
        ...
        /// 獲取指定文件详细屬性
        /// </summary>
        /// <param name="filePath">文件详细路徑</param>
        /// <returns></returns>
        public static string GetFileAttibe(string filePath)
        {
            ...
            str += "详细路徑:" + objFI.FullName + "<br>文件名称:" + objFI.Name + "<br>文件长度:" + objFI.Length.ToString() + "字节<br>创建时间" + objFI.CreationTime.ToString() + "<br>最后訪問时间:" + objFI.LastAccessTime.ToString() + "<br>修改时间:" + objFI.LastWriteTime.ToString() + "<br>所在目錄:" + objFI.DirectoryName + "<br>擴展名:" + objFI.Extension;
            return str;
        }
