...
namespace VOL.Core.Utilities
{
    public class EPPlusHelper
    {
        ...
        /// <param name="ignoreColumns">忽略不导出的列(如果設置了exportColumns,ignoreColumns不会生效)</param>
            ...
            )
        {
            ...
            if (!file.Exists) return responseContent.Error("未找到上传的文件,請重新上传");
            ...
            using (ExcelPackage package = new ExcelPackage(file))
            {
                ...
                //2020.08.11修复獲取表結构信息时，表為别名时查不到数据的問题
                ...
                {
                    return responseContent.Error("导入文件列必須與导入模板相同");
                }
                ...
                for (int m = sheet.Dimension.Start.Row + 1, n = sheet.Dimension.End.Row; m <= n; m++)
                {
                    ...
                    for (int j = sheet.Dimension.Start.Column, k = sheet.Dimension.End.Column; j <= k; j++)
                    {
                        ...
                        if (string.IsNullOrEmpty(value))
                        {
                            if (options.Requierd)
                            {
                                return responseContent.Error($"第{m}行[{options.ColumnCNName}]验证未通過,不能為空。");
                            }
                            continue;
                        }
                        ...
                        if (!string.IsNullOrEmpty(options.DropNo) && !string.IsNullOrEmpty(value)&&!ignoreSelectValidationFields.Contains(property.Name))
                        {
                            ...
                            {
                                return responseContent.Error($"[{options.ColumnCNName}]字段数字典编号[{options.DropNo}]缺失,請检查字典配置");
                            }
                            string key = null;
                            //2022.11.21增加导入多选的支持
                            ...
                            if (key == null)//&& options.Requierd
                            {
                                ...
                                return responseContent.Error($"第{m}行[{options.ColumnCNName}]验证未通過,必須是字典数据中[{values}]的值。");
                            }
                            //將值設置為数据字典的Key,如果导入為是/否字典项，存在表中應该对為1/0
                            value = key;
                        }
                        ...
                        if (!result.Item1)
                        {
                            return responseContent.Error($"第{m}行[{options.ColumnCNName}]验证未通過,{result.Item2}");
                        }
        ...
        /// <param name="exportColumns">指定导出的列</param>
        /// <param name="ignoreColumns">忽略不导出的列(如果設置了exportColumns,ignoreColumns不会生效)</param>
        /// <param name="savePath">导出文件的绝对路徑</param>
        ...
        /// <param name="cellOptions">对應代码生成器的配置</param>
        ...
        public static string Export<T>(List<T> list, IEnumerable<string> exportColumns, IEnumerable<string> ignoreColumns, string savePath, string fileName, bool template = false)
        {
            ...
            //2021.01.24修复多选類型，导出excel文件没有转换数据源的問题
            ...
            /*导出时，代码生成器中的表配置信息Sys_TableInfo/Sys_TableColumn必須與當前數據庫相同，否则导出来可能没有数据*/
            ...
            if (exportColumns != null && exportColumns.Count() > 0)
            {
                ...
                //.Where(x => cellOptions.Select(s => s.ColumnName) //獲取代码生成器配置的列
            ...
            else if (ignoreColumns != null && ignoreColumns.Count() > 0)
            {
                propertyInfo = typeof(T).GetProperties()
                  ...
                  .Where(x => cellOptions.Select(s => s.ColumnName).Contains(x.Name)) //獲取代码生成器配置的列
                  .ToList();
            }
            else
            {
                //默認导出代码生成器中配置【是否显示】=是的列
                propertyInfo = typeof(T).GetProperties()
                  .Where(x => cellOptions.Select(s => s.ColumnName).Contains(x.Name)) //獲取代码生成器配置的列
                  .ToList();
                /*
                 * 如果propertyInfo查出来的长度=0
                 * 1、代码生成器中的配置信息是否同步到當前數據庫
                 * 2、代码生成器中的配置列名與model的字段是否大小写一致
                 */
            }
            string[] dateArr = null;
            if (!template)
        ...
        {
            //&& x.IsDisplay == 1&&x.IsReadDataset==0只导出代码生器中設置為显示并且不是只读的列，可根据具体業務設置导出列
            // && x.IsReadDataset == 0
            ...
            foreach (string dicNo in dicNos.Distinct())
            {
                ...
                {
                    ////filterKeyValue為true過滤keyvalue相不的项,key==value相同的则不處理
    ...
    public class CellOptions
    {
        ...
        //public string Value { get; set; } //对應字典项维护的Value
        //public string Name { get; set; } //对應字典项显示的名称
    }
}
