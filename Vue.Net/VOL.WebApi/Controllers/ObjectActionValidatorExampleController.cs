...
{
    /// <summary>
    ///1、普通参数校验只需要标识屬性：[ObjectGeneralValidatorFilter(ValidatorGeneral.xxx,ValidatorGeneral.xxx)]，
    ///需要在ValidatorGeneral枚举中添加枚举值(参数名)，并在UseMethodsGeneralParameters方法中注入进去即可在任何地方重复使用
    /// 
    /// 2、model校验只需要标识屬性[ObjectModelValidatorFilter(ValidatorModel.xxx)]
    ...
    public class ObjectActionValidatorExampleController: Controller
    {
        ...
        /// <summary>
        /// 同时验证实体LoginInfo與单个参数phoneNo
        ///  Login配置的规则用戶名與密码必填,手机号必填
        /// </summary>
        /// <param name="loginInfo"></param>
        /// <returns></returns>
