...
namespace VOL.System.Controllers
{
    ...
    {
        private readonly ISys_WorkFlowService _service;//訪問業務代码
        ...
        public async Task<IActionResult> GetSteps([FromBody] List<string> ids, string tableName)
        {
            ...
            // 獲取按用戶审核的id，如果多用戶要进行分割
            // 转换成int数組
            //var auditUsers = flow.Sys_WorkFlowTableStep
            //    .Where(x => x.StepType == (int)AuditType.用戶审批 && x.StepValue != null)
            //    .SelectMany(x => x.StepValue.Split(",")).Select(int.Parse).ToArray();

            //未审批时的用戶信息
            var unauditSteps = flow.Sys_WorkFlowTableStep
                .Where(x => (x.AuditId == null || x.AuditId == 0) && x.StepType == (int)AuditType.用戶审批)
            ...
            var steps = flow.Sys_WorkFlowTableStep
                    .Select(c => new
                    {
                        ...
                        isCurrent = c.StepId == flow.CurrentStepId && c.AuditStatus != (int)AuditStatus.审核通過
                    }).OrderBy(o => o.OrderId);
