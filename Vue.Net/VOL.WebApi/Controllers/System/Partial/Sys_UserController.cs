...
namespace VOL.System.Controllers
{
    ...
    public partial class Sys_UserController
    {
        ...
        public IActionResult ReplaceToken()
        {
            ...
            try
            {
                //如果5秒内替换過token,直接使用最新的token(防止一个页面多个并发請求同时替换token导致token错位)
                ...
                lock (_obj)
                {
                    ...
                    if (userInfo == null) return Json(responseContent.Error("未查到用戶信息!"));
            ...
            finally
            {
                ...
                string _message = $"用戶{userInfo?.User_Id}_{userInfo?.UserTrueName},({(responseContent.Status ? "token替换成功": "token替换失败")})";
        ...
        [ApiActionPermission]
        //通過ObjectGeneralValidatorFilter校验参数，不再需要if esle判断OldPwd與NewPwd参数
        ...
        public IActionResult ModifyUserPwd(string password, string userName)
        {
            ...
            if (user == null)
            {
                return Json(webResponse.Error("用戶不存在"));
            }
