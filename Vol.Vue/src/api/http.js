...
function showLoading (loading) {
    ...
    loadingInstance = Loading.service({
        ...
        background: typeof loading == "string" ? loading : '正在處理.....',
...
/*
  url
  params請求后台的参数,如：{name:123,values:['a','b','c']}
    ...
    return new Promise((resolve, reject) => {
        axios.post(url, params, config)
            .then(response => {
                ...
                reject(err && err.data && err.data.message ? err.data.message : '服务器處理异常');
            })
            .catch((error) => {
                reject(error)
            })
...
//=true异步請求時会显示遮罩层,=字符串，异步請求時遮罩层显示當前字符串
