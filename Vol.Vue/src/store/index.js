...
export default createStore({
  state: {
    data: {},
    permission: [],
    isLoading: false,//2020.06.03增加路由切换時加载提示
    userInfo: null
  },
  mutations: {
    setPermission(state, data) {  //調用方式 this.$store.commit('setPermission', data)
    ...
    getPermission: (state) => (path) => {  //調用方式 store.getters.getPermission('sys_User')
