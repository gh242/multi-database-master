<template>
    ...
    >
      ...
      >
        ...
        <template #default="scope">
          <!-- 2022.01.08增加多表头，现在只支持常用功能渲染，不支持编辑功能(涉及到組件重写) -->
          <el-table-column
            style="border: none"
          ...
          <template v-else>
            ...
            ></div>
            <!-- 2021.11.18修复table数据源設置為normal后点击行$event缺失的問题 -->
            <div
  ...
  <VolBox
    ...
    lazy
  >
    <!-- 上传圖片、excel或其他文件、文件数量、大小限制都可以，参照volupload組件api -->
...
export default defineComponent({
  ...
  props: {
    ...
    index: {
      // 是否创建索引号,如果需要表格编辑功能，這裡需要設置為true
      type: Boolean,
      default: false
    },
    allowEmpty: {
  ...
  data() {
    return {
      fixed: false, //是固定行号與checkbox
      clickEdit: true, //2021.07.17設置為点击行結束编辑
      randomTableKey: 1,
      visiblyColumns: [],
      key: '',
      realHeight: 0,
      ...
      errorFiled: '',
      edit: { columnIndex: -1, rowIndex: -1 }, // 當前双击编辑的行與列坐标
      editStatus: {},
        ...
        .then((dic) => {
          dic.forEach((x) => {
            ...
            columnBind.forEach((c) => {
              // 转换数据源的類型與列的類型一致(2020.04.04)
              if (
                c.key == x.dicNo &&
                (c.valueTyoe == 'int' || c.valueTyoe == 'sbyte')
              ) {
                x.data.forEach((d) => {
                  // 2020.09.01增加对数字類型的二次判断
                  if (!isNaN(d.key)) {
                    d.key = ~~d.key;
                  }
                });
  ...
  methods: {
    ...
    rowClick(row, column, event) {
      //2022.02.20增加点击時表格参数判断
      if (!column) {
        return;
      }
      //正在编辑時，禁止出发rowClick事件
      ...
      // 点击其他行時，如果点击的行與正在编辑的行相同，保持编辑状态
      if (this.clickEdit && this.edit.rowIndex != -1) {
        if (row.elementIndex == this.edit.rowIndex) {
          // 点击的单元格如果不可以编辑，直接結束编辑
          // 2020.10.12修复結束编辑時，element table高版本屬性獲取不到的問题
        ...
        //當正在编辑，且点击到其他行時，在原编辑的行結束编辑后，触发新行的rowClick事件
        //正在编辑時，禁止出发rowClick事件
        if (this.edit.rowIndex == -1) {
          this.$emit('rowClick', { row, column, event });
        }
    ...
    rowEndEdit(row, column, event) {
      ...
      if (this.edit.rowIndex != -1) {
        //2022.06.26修复表格内容切换后行数不一致時不能编辑的問题
    ...
    addRow(row) {
      ...
      this.columns.forEach((x) => {
        // 2022.05.06 添加行時，如果列有编辑屬性，設置开启编辑(避免关闭编辑后，无法再次启用编辑)??
        //x.readonly = false;
    ...
    load(query, isResetPage) {
      ...
      let param = {
        ...
        wheres: [] // 查询条件，格式為[{ name: "字段", value: "xx" }]
      };
      let status = true;
      ...
      /* 查询前處理(如果需要查询条件，实现組件方法loadBefore方法即可:
    ...
    formatter(row, column, template) {
      ...
      let source = column.bind.data.filter((x) => {
        ...
        // 2020.06.06修复单独使用table組件時,key為数字0時转换成文本失败的問题
    ...
    getSelectFormatter(column, val) {
      ...
      for (let index = 0; index < valArr.length; index++) {
        ( column.bind.orginData&&column.bind.orginData.length
          ...
          :column.bind.data)
        .forEach((x) => {
          // 2020.06.06修复数据源為selectList時,key為数字0時不能转换文本的問题
