		...
		remote
            ...
            >
              <el-date-picker
                ...
                :type="item.type"
                :placeholder="
                  item.placeholder ? item.placeholder : '請选择' + item.title
                "
                :disabledDate="(val) => getDateOptions(val, item)"
                :value-format="getDateFormat(item)"
              >
            ...
            <el-input
              ...
              :placeholder="item.placeholder ? item.placeholder : item.title"
            />
            <!-- 2021.11.18修复el-input没有默認enter事件時回车异常 -->
            <el-input
              :size="size"
              clearable
              :ref="item.field"
...
//表单验证注意：每次验证都必須執行callback,否则验证不執行回調方法
  ...
  setup(props, context) {
    ...
    const validate =async (callback) => {
      ...
      await volform.value.validate((valid) => {
        if (!valid) {
          appContext.config.globalProperties.$message.error('数据验证未通過!');
          result = false;
        } else if (typeof callback === 'function') {
          try {
            callback(valid);
          } catch (error) {
            let msg = `表单验证回調方法异常：${error.message}`;
  ...
  methods: {
    getColWidth(item) {
      ...
      this.formRules.forEach((row, xIndex) => {
        //2022.05.06 追加表单中隐藏的元素不参與动态计算表单宽度
    ...
    // 是否為圖片文件等格式并对字段的转换成数組：[{name:'1.jpg',path:'127.0.0.1/ff/1.jpg'}]
    ...
    convertFileToArray(item, formFields) {
      if (!item.maxFile) {
        item.maxFile = 1; // 默認只能上传一个文件，可以在onInit中設置
      }
      ...
      // 將以逗号隔开的文件分割成数組127.0.0.1/aa/1.jpg,將127.0.0.1/aa/2.jpg
    ...
    validatorPhone(ruleOption, value, callback) {
      ...
      if (!rule.phone.test((value || '').trim())) {
        return callback(new Error('請输入正确的手机号'));
      }
      callback();
    },
    ...
    convertArrayValue(data, val) {
      ...
      // 编辑多选table显示
      //2023.04.20修复只读為label時原数据被字典替换了的問题
    ...
    getText(formFields, item) {
      // 2019.10.24修复表单select組件為只读的屬性時没有绑定数据源
      ...
      //2021.03.31修复表单switch只读時没有转换值的問题
      if (item.type == 'switch') {
        return text ? '是' : '否';
      }
    ...
    onRemoteChange(item, value) {
      // 第二次打开時，默認值成了undefined，待查viewgrid中重置代码
    ...
    remoteSearch(item, formFields, val) {
      ...
      // 弹出框或初始化表单時给data設置数組默認值2
      // 2020.09.26修复远程搜索自定义url不起作用的問题
      let url;
      ...
      if (item.validator && typeof item.validator === 'function') {
        return {
          validator: (rule, val, callback) => {
            // 用戶自定义的方法，如果返回了值，直接显示返回的值，验证不通過
        ...
        return {
          validator: (rule, val, callback) => {
            ...
            if (
              ...
              return callback(
                new Error(item.type == 'img' ? '請上传照片' : '請上传文件')
              );
            }
            return callback();
          },
        ...
        return {
          ...
          validator: (rule, val, callback) => {
            ...
            if (!val || (item.range && !val.length)) {
              return callback(new Error('請选择日期'));
            }
            return callback();
          }
        };
      ...
      if (item.type == 'cascader') {
        return {
          ...
          validator: (rule, val, callback) => {
            ...
            if (!_arr || !_arr.length) {
              return callback(new Error('請选择' + item.title));
            }
            return callback();
          }
        };
