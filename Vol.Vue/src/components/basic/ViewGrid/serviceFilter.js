...
let serviceFilter = {
  ...
  onInited () { //对應created，在onInit與onInited中间会初始化界面数据对象
  ...
  searchAfter (param, result) { //查询ViewGird表数据后param查询参数,result回返查询的結果
  ...
  searchDetailAfter (param, data) {//查询从表后param查询参数,result回返查询的結果
    // console.log(this.detailOptions.cnName + '触发loadDetailTableAfter');
    return true;
  },
  delBefore (ids, rows) { //查询界面的表删除前 ids為删除的id数組,,rows删除的行
    return true;
  },
  ...
  addBefore (formData) { //新建保存前formData為对象，包括明细表
    return true;
  },
  async addBeforeAsync (formData) { //异步處理,功能同上(2020.12.06)
    return true;
  },
  addAfter (result) {//新建保存后result返回的状态及表单对象
    return true;
  },
  updateBefore (formData) { //编辑保存前formData為对象，包括明细表、删除行的Id
    return true;
  },
  async updateBeforeAsync (formData) { //异步處理,功能同上(2020.12.06)
    return true;
  },
  ...
  modelOpenBefore (row) { //点击编辑/新建按钮弹出框前，可以在此處写逻辑，如，从后台獲取数据

  },
  modelOpenAfter (row) {  //点击编辑/新建按钮弹出框后，可以在此處写逻辑，如，从后台獲取数据
  ...
  exportBefore (param) { //2020.06.25增加导出前處理
    return true;
  },
  onModelClose(iconClick){
    //iconClick=true為点击左中上角X触发的关闭事件
    //如果返回 false不会关闭弹出框 
    //return false;
    this.boxModel=false;
  },
