...
let methods = {
  //當添加擴展組件gridHeader/gridBody/gridFooter及明细modelHeader/modelBody/modelFooter時，
  //如果要獲取父级Vue对象,請使用此方法进行回調
  parentCall(fun) {
    if (typeof fun != 'function') {
      return console.log('擴展組件需要传入一个回調方法才能獲取父级Vue对象');
    }
    fun(this);
  },
  getCurrentAction() {
  ...
  extendBtn(btns, source) {
    //btns权限按钮，source為擴展按钮
    if (!btns || !(source && source instanceof Array)) {
      return;
    }
    //source通過在表的擴展js文件中buttons对應按钮的屬性index决定按钮所放位置
    source.forEach((x) => {
      //通過按钮的Index屬性，放到指定的位置
  ...
  initBoxButtons() {
    //初始化ViewGird與弹出框/明细表按钮
    let path = this.$route.path;
    //通過菜单獲取用戶所对應菜单需要显示的按钮
    ...
    //查询界面擴展按钮(擴展按钮可自行通過設置按钮的Index屬性显示到具体位置)
    if (this.extend.buttons.view) {
    ...
    // disabled
    //如果當前角色没有编辑或新建功能，查看明细時字段設置全部只读
    //只有明细表，將明细表也設置為不可能编辑，并且不显示添加行、删除行
    if (!saveBtn) {
      ...
      //弹出弹框按钮(2020.04.21),没有编辑或新建权限時，也可以通過buttons屬性添加自定义弹出框按钮
    ...
    this.detailOptions.buttons.push(
      ...[
        ...
        //注意需要重写后台明细表接口的导入與下载模板、导出的权限,Sys_DictionaryListController.cs/SellOrderListController.cs
        {
          type: 'danger',
          plain: true,
          name: '导入',
  ...
  getSearchParameters() {
    //獲取查询参数
    // 2020.09.11增加固定查询表单,如果設置固定了查询表单，点击查询時，不再关闭
    if (!this.fiexdSearchForm) {
    ...
    for (const key in this.searchFormFields) {
      ...
      if (displayType == 'cascader') {
        //查询下面所有的子节点，如：选中的是父节点，應该查询下面所有的节点数据--待完
  ...
  resetForm(formName, sourceObj) {
    ...
    //獲取数据源的data類型，否则如果数据源data的key是数字，重置的值是字符串就无法绑定值
    ...
    for (const key in form) {
      if (sourceObj.hasOwnProperty(key)) {
        ...
        if (
          ...
          // 2020.11.01增加iview組件Cascader表单重置時查询所有的父节点
  ...
  async saveExecute() {
    ...
    if (_currentIsAdd) {
      //2020.12.06增加新建前异步處理方法
      //2021.08.16修复异步语法写错的問题
      if (!this.addBefore(formData) || !(await this.addBeforeAsync(formData)))
        return;
    } else {
      //2020.12.06增加修改前异步處理方法
      if (
    ...
    this.http.post(url, formData, true).then((x) => {
      ...
      //如果保存成功后需要关闭编辑框，直接返回不處理后面
      if (this.boxOptions.saveClose) {
        this.boxModel = false;
        //2020.12.27如果是编辑保存后不重置分页页数，刷新页面時还是显示當前页的数据
  ...
  del(rows) {
    ...
    if (!rows || rows.length == 0) return this.$error('請选择要删除的行!');
    let delKeys = rows.map((x) => {
      return x[this.table.key];
    });
    if (!delKeys || delKeys.length == 0)
      return this.$error('没有獲取要删除的行数据!');
    //删除前
  ...
  async linkData(row, column) {
    ...
    //設置远程查询表单的默認key/value
    this.getRemoteFormDefaultKeyValue();
    //点击编辑按钮弹出框后，可以在此處写逻辑，如，从后台獲取数据
  ...
  import() {
    //导入(上传excel),弹出导入組件UploadExcel.vue
    this.upload.excel = true;
  ...
  audit() {
    ...
    if (rows.length == 0) return this.$error('請选择要审核的行!');
    let auditStatus = Object.keys(rows[0]).find(x => { return x.toLowerCase() === 'auditstatus' });
    if (!auditStatus) {
      return this.$message.error(`表必須包括审核字段【AuditStatus】,并且是int類型`)
    }
  ...
  initFormOptions(formOptions, keys, formFields, isEdit) {
    //初始化查询、编辑对象的下拉框数据源、圖片上传链接地址
    //let defaultOption = { key: "", value: "請选择" };
    //有上传的字段
    //2020.05.03新增
    //编辑数据源的類型
    formOptions.forEach((item) => {
      item.forEach((d) => {
        ...
        if (keys.indexOf(d.dataKey) == -1) {
          //2020.05.03增加记錄编辑字段的数据源類型

          keys.push(d.dataKey);
          //2020.05.03修复查询表单與编辑表单type類型變成强一致性的問题
          //this.dicKeys.push({ dicNo: d.dataKey, data: [], type: d.type });
          //  2020.11.01增加iview組件Cascader数据源存儲
          let _dic = {
            dicNo: d.dataKey,
            data: [],
            fileds: [d.field],
        ...
        if (d.type != 'cascader') {
          //2020.01.30移除内部表单formOptions数据源配置格式data.data，所有参数改為與組件api格式相同
          Object.assign(
            d,
  ...
  //初始table與明细表的数据源指向dicKeys对象，再去后台加载数据源
  initColumns(scoure, dicKeys, keys) {
    ...
    scoure.forEach((item) => {
      ...
      //2020.05.03优化table数据源checkbox與select類型从编辑列中选取
      ...
      this.dicKeys.forEach((x) => {
        ...
        if (x.type == 'cascader' || x.type == 'treeSelect') {
          ...
          for (let index = 0; index < source.length; index++) {
            newSource[index] = {
              //默認从字典数据读出来的key都是string類型,但如果数据从sql中查询的可能為非string,否是async-validator需要重置設置格式
  ...
  initDicKeys() {
    ...
    //初始化编辑数据源,默認為一个空数組，如果要求必填設置type=number/decimal的最小值
    ...
    //初始化datatable表数据源,默認為一个空数組,dicKeys為界面所有的数据字典编号
    this.initColumns(this.columns, this.dicKeys, keys);
    //2021.05.23默認开启查询页面所有字段排序,如果不需要排序，在onInited遍历columns設置sort=false
    //2021.09.25移除强制排序功能
  ...
  setFiexdColumn(columns, containerWidth) {
    //计算整个table的宽度，根据宽度决定是否启用第一行显示的列為固定列
    //2021.09.21移除强制固定第一列
    // let columnsWidth = 0;
    // columns.forEach(x => {
    //   if (!x.hidden && x.width) {
  ...
  initBoxHeightWidth() {
    ...
    if (
      ...
      if (this.boxOptions.height) {
        //如果高度與宽度超過了獲取到的可见高宽度，则設為默認的90%高宽
    ...
    if (this.detail.columns && this.detail.columns.length > 0) {
      ...
      //计算弹出框整个table的宽度，根据宽度决定是否启用第一行显示的列為固定列
  ...
  setFiexdSearchForm(visiable) {
    //2020.09.011增加固定查询表单功能,visiable=true默認將查询表单展开
  ...
  tableBeginEdit(row, column, index) {
    //2021.03.19是否开启查询界面表格双击编辑結束方法,返回false不会結束编辑
  ...
  destroyed() {
    //2021.04.11增加vue页面销毁方法,路由必須設置keepLive:false，設置方法见：前端开发文档-》[禁用页面緩存keepAlive]
  },
  ...
  loadTreeChildren(tree, treeNode, resolve) {
    //树形結构加载子节点(2021.05.02),在onInit中設置了rowKey主键字段后才会生效
    return resolve([]);
  },
  importDetailAfter(data) {
    //2022.01.08增加明细表导入后處理
  },
  importExcelAfter(data) {
    ...
    if (this.boxModel) {
      ...
      this.importDetailAfter(data); //增加明细表导入后處理
