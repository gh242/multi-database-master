  ...
  <div class="layout-container">
    ...
    >
      <template #content>
        <el-tabs type="card">
          ...
          <el-tab-pane v-if="workFlowSteps.length" label="审批记錄">
    ...
    <div class="view-container">
      ...
      <div class="grid-search">
        ...
        >
          <!-- 2020.09.13增加formFileds拼写错误兼容處理 -->
          <vol-form
            ref="searchForm"
            :load-key="false"
...
var vueParam = {
  ...
  data() {
    return {
      ...
      formFieldsType: [],
      workFlowSteps: [],
      //树形結构的主键字段，如果設置值默認会开启树形table；注意rowKey字段的值必須是唯一（2021.05.02）
      rowKey: undefined,
      fiexdSearchForm: false, //2020.09.011是否固定查询表单，true查询表单將固定显示在表单的最上面
      _inited: false,
      doubleEdit: false, //2021.03.19是否开启查询界面表格双击编辑
      single: false, //表是否单选
      const: _const, //增删改查导入导出等对應的action
      boxInit: false, //新建或编辑的弹出框初化状态，默認不做初始化，点击新建或编辑才初始化弹出框
      searchBoxShow: false, //高级查询(界面查询后的下拉框点击触发)
      singleSearch: {}, //快速查询字段
      exportHref: '',
      currentAction: _const.ADD, //當新建或编辑時，记錄當前的状态:如當前操作是新建
      currentRow: {}, //當前编辑或查看数据的行
      closable: false,
      boxModel: false, //弹出新建、编辑框
      width: 700, //弹出框查看表数据結构
      labelWidth: 100, //高级查询的标签宽度
      viewModel: false, //查看表結构的弹出框
      viewColumns: [], //查看表結构的列数据
      viewColumnsClone: [],
      showCustom: true, //是否显示自定义配置列按钮2022.05.27
      // viewData: [], //查看表結构信息
      maxBtnLength: 8, //界面按钮最多显示的个数，超過的数量都显示在更多中
      buttons: [], //查询界面按钮  如需要其他操作按钮，可在表对應的.js中添加(如:Sys_User.js中buttons添加其他按钮)
      splitButtons: [],
      uploadfiled: [], //上传文件圖片的字段
      boxButtons: [], //弹出框按钮 如需要其他操作按钮，可在表对應的.js中添加
      dicKeys: [], //當前界面所有的下拉框字典编号及数据源
      ...
      initActivated: false,
      load: true, //是否默認加载表数据
      activatedLoad: false, //页面触发actived時是否刷新页面数据
      summary: false, //查询界面table是否显示合计
      //需要从远程绑定数据源的字典编号,如果字典数据源的查询結果较多，請在onInit中將字典编号添加进来
      //只对自定sql有效
      remoteKeys: [],
      columnIndex: false, //2020.11.01是否显示行号
      ck: true, //2020.11.01是否显示checkbox
      continueAdd: false, //2021.04.11新建時是否可以连续新建操作
      continueAddName: '保存后继续添加', //2021.04.11按钮名称
      // detailUrl: "",
      detailOptions: {
        //弹出框从表(明细)对象
        //从表配置
        buttons: [], //弹出框从表表格操作按钮,目前有删除行，添加行，刷新操作，如需要其他操作按钮，可在表对應的.js中添加
        cnName: '', //从表名称
        key: '', //从表主键名
        data: [], //数据源
        columns: [], //从表列信息
        ...
        load: false, //
        delKeys: [], //當编辑時删除當前明细的行主键值
        url: '', //从表加载数据的url
        pagination: { total: 0, size: 100, sortName: '' }, //从表分页配置数据
        height: 0, //默認从表高度
        textInline: true, //明细表行内容显示在一行上，如果需要换行显示，請設置為false
        doubleEdit: true, //使用双击编辑
        clickEdit: false, //是否开启点击单元格编辑，点击其他行時結束编辑
        currentReadonly: false, //當前用戶没有编辑或新建权限時，表单只读(可用于判断用戶是否有编辑或新建权限)
        //开启编辑時
      ...
      auditParam: {
        ...
        showViewButton: true,
        auditHis: [],
        showAction: false, //是否显示审批操作(當前节点為用戶审批時显示)
        //审核选项(可自行再添加)
        data: [
          { text: '通過', value: 1 },
          { text: '拒绝', value: 2 },
      ...
      upload: {
        ...
        url: '', //导入的路徑,如果没有值，则不渲染导入功能
        template: {
          //下载模板对象
          url: '', //下载模板路徑
          fileName: '' //模板下载的中文名
        },
        init: false //是否有导入权限，有才渲染导入組件
      },
      height: 0, //表高度
      ...
      select2Count: 500 //超出500数量显示select2組件
    };
  },
  methods: {},
  activated() {
  ...
  created: function() {
    ...
    //如果没有指定排序字段，则用主键作為默認排序字段
    this.pagination.sortName = this.table.sortName || this.table.key;
    this.initBoxButtons(); //初始化弹出框與明细表格按钮
    this.initAuditColumn();
    this.onInit(); //初始化前，如果需要做其他處理在擴展方法中覆盖此方法
    this.getButtons();
    //初始化自定义表格列
    ...
    this.onInited(); //初始化后，如果需要做其他處理在擴展方法中覆盖此方法
  },
