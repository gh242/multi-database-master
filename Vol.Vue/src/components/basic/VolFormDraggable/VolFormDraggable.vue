    ...
    <div class="drag-center">
      <div class="center-top">
        ...
        <a
          ...
          target="_blank"
          >設计器基于框架volform、voltable、volupload、volbox定制开发</a
        >
      </div>
      <div>
        <el-alert
        ...
        >
          <div>
            1、表单設计器基于draggable开发,為本框架自定义页面功能的补充,框架仍以可視化代码生成器為核心
          </div>
          <div>
            2、支持可視化設计1对1、1对多及表单下拉框自动绑定、table自动加载数据(分页、编辑)、自动上传文件、富文本编辑
          </div>
        </el-alert>
      </div>
    ...
    <div class="drag-right">
      ...
      <div class="attr" v-show="currentIndex != -1">
        ...
        <div class="attr-item" v-show="currentItem.data">
          <div class="text">数据源(下拉框绑定設置中维护)</div>
  ...
  >
    <template #content>
      ...
      <el-alert title="关于table配置" type="info" :closable="false" show-icon>
        此處table是对框架voltable基本操作的配置,如果需要事件触发、数据加载等更多功能，請在生成后的代码添加需要的功能，完整配置见文档<a
          href="http://v2.volcore.xyz/document/api"
