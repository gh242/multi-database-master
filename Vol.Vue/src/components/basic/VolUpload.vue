...
export default {
  ...
  props: {
    ...
    fileInfo: {
      //用于接收上传的文件，也可以加以默認值，显示已上传的文件，用戶上传后会覆盖默認值
      type: Array,
      default: () => {
        return [];
    ...
    img: {
      //圖片類型  img>excel>fileTypes三种文件類型优先级
      type: Boolean,
      default: false
    },
    excel: {
  ...
  data() {
    return {
      ...
      changed: false, //手动上传成功后禁止重复上传，必須重新选择
      model: true,
      files: [],
      bigImg: '',
  ...
  methods: {
    ...
    async uploadOSS() {
      await this.http.get('api/alioss/getAccessToken', {}, false).then(async (x) => {
        ...
        let client = new OSS({
          // yourRegion填写Bucket所在地域。以华东1（杭州）為例，Region填写為oss-cn-hangzhou。
          region: x.data.region,
          // 从STS服务獲取的临時訪問密钥（AccessKey ID和AccessKey Secret）。
          accessKeyId: x.data.accessKeyId,
          accessKeySecret: x.data.accessKeySecret,
          // 从STS服务獲取的安全令牌（SecurityToken）。
    ...
    async upload(vail) {
      ...
      if (!this.files || this.files.length == 0) {
        return this.$message.error('請选择文件');
      }
      //增加上传時自定义参数，后台使用獲取Utilities.HttpContext.Current.Request.Query["字段"]
      let params={};
    ...
    checkFile(inputFiles) {
      ...
      for (let index = 0; index < inputFiles.length; index++) {
        ...
        if (this.img && !this.format(file, 'img')) {
          this.$message.error('选择的文件【' + file.name + '】只能是圖片格式');
          return false;
        }
