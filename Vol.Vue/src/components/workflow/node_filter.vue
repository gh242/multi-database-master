<template>
    <div class="node-filter-container">
        <!-- <div class="add-btn">
          <span class="name">条件設置</span>  <el-button @click="addItem" link><i>+</i>添加字段</el-button>
        </div> -->
        <!-- {{ $store.getters.data().flowTable.WorkTable }} -->
        <div class="ef-node-pmenu-item" style="display: flex;">
            <div style="flex:1;">
                <span class="name"><i class="el-icon-news"></i>条件設置</span>
            </div>
            ...
            <table>
                ...
                <tr v-for="(item, index) in filters" :key="index">
                    ...
                    <td><el-select @change="(field) => { fieldChange(field, index) }" size="small" v-model="item.field"
                            placeholder="請选择" :disabled="disabled">
                            <el-option v-for="data in fieldsOptions" :key="data.field" :label="data.name"
                                :value="data.field" />
                        </el-select></td>
                    <td><el-select size="small" v-model="item.filterType" placeholder="請选择" :disabled="disabled">
                    ...
                    <td>
                        <template v-if="item.data">
                            ...
                            <el-select-v2 style="width: 100%;" v-else multiple size="small" :options="item.data"
                                v-model="item.value" placeholder="請选择" :disabled="disabled">
                            </el-select-v2>
                        </template>
