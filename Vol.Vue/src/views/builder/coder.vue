      ...
      <div style="height: 100%">
        <el-scrollbar style="height: 100%">
          <div class="coder-container">
            ...
            <el-alert type="warning" :closable="false">
              1、如果需要修改表結构，請在數據庫修改，再点同步表結构->生成vue页面->生成model。
...
export default {
  ...
  methods: {
    ...
    delTree() {
      ...
      if (!tableId) return this.$message.error('請选择节点');
      let tigger = false;
    ...
    addChild() {
      ...
      if (!id) {
        return this.$message.error('請选中节点');
      }
      this.addVisible(id);
    },
    addRow() {
    ...
    validateTableInfo(callback) {
      this.$refs.form.validate(() => {
        ...
        if (this.data && this.data.length > 0) {
          ...
          if (!keyInfo) {
            this.$message.error('請勾选設置主键');
            return false;
          }
          if (keyInfo.isNull == 1) {
            this.$message.error('主键【可為空】必須設置為否');
            return false;
          }
          if (
            ...
            this.$message.error('主键非自增類型,請設置上面表单的【排序字段】');
            return false;
          }
        }
        ...
        if (!isApp) {
          ...
          if (!vuePath) {
            return this.$message.error(
              '請先設置Vue项目对應Views的绝对路徑,然后再保存!'
            );
          }
        } else {
          vuePath = localStorage.getItem('appPath');
          if (!vuePath) {
            return this.$message.error('請先設置app路徑,然后再保存!');
          }
        }
    ...
    syncTable() {
      if (!this.layOutOptins.fields.tableName)
        return this.$Message.error('請选模块');
      this.http
        .post(
    ...
    save() {
      ...
      if (
        ...
        return this.$Message.error('表結构只能勾选一个主键字段');
      }
      this.validateTableInfo(() => {
  ...
  created() {
    ...
    this.http.post('/api/builder/GetTableTree', {}, false).then((x) => {
      ...
      if (!x.nameSpace) {
        return this.$message.error(
          '未獲取后台项目類庫所在命名空间,請确认目錄或調试Sys_TableInfoService類GetTableTree方法'
        );
      }
