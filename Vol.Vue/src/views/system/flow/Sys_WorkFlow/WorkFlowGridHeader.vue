...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      if (!nodes.length) {
        return this.$message.error('請添加流程节点');
      }
      //獲取根节点
      ...
      if (root.length > 1) {
        return this.$message.error('只能設置一个流程根节点');
      }
      let detailData = [];
      ...
      //查找當前节点的下一个节点(后面增加会签等分支的時候,find改為filter批量查找)
      ...
      for (let index = 0; index < detailData.length; index++) {
        ...
        if (!step.StepName) {
          return this.$message.error(`請输入第【${index + 1}】个节点的名称`);
        }
        // if (!step.StepType) {
        //    return this.$message.error(`請输入第【${index + 1}】个节点的名称`);
        // }
