        ...
        <template #header>
          <div>
            <el-alert show-icon
                      type="success">
              配置的其他权限
              <br />1、添加新的权限后請在vue项目中config文件夹下buttns.js添加此权限的按钮。
              <br />2、如果权限只在某少数几个功能中使用,在vue的对應页面擴展extension文件找到对應js,添加到el-buttons对象中,格式同config文件夹下buttns.js一樣。
            </el-alert>
          </div>
        </template>
      </vol-form>
    ...
    <div class="menu-right">
      <el-scrollbar style="height: 100%">
        <el-alert title="菜单配置说明"
                  ...
                  show-icon>
          <div>
            1、如果是用代码生器生成的Vue页面,Url為Vue项目中src->router->viewGrid.js对應表名的path屬性
          </div>
          <div style="padding-top: 5px">
            2、 如果只是建一级菜单或空菜单url不用填写,【視圖/表名】填写.或者/
          </div>
        </el-alert>
  ...
  methods: {
    ...
    delMenu () {
      ...
      if (this.fields.menu_Id == 0) {
        return this.$Message.error("請选择菜单");
      }
