<template>
  <div class="login-container">
    ...
    <div class="login-form">
      <div class="form-user" @keypress="loginPress">
        ...
        <div class="item">
          ...
          <input type="text" v-focus v-model="userInfo.userName" placeholder="請输入账号" />
        </div>
        <div class="item">
          <div class="input-icon el-icon-lock"></div>
          <input type="password" v-focus v-model="userInfo.password" placeholder="請输入密码" />
        </div>
        <div class="item">
    ...
    <div class="login-footer">
      ...
      <a href="https://space.bilibili.com/525836469" style="text-decoration: none" target="blank">NET視频教程(微软MVP-ACE錄制)</a>
      <a href="https://www.cctalk.com/m/group/90268531" style="text-decoration: none" target="blank">VOL框架視频</a>
      <a href="http://120.48.115.252:9990" style="text-decoration: none" target="blank">視频演示地址</a>
    </div>
...
export default defineComponent({
  setup(props, context) {
    ...
    const login = () => {
      if (!userInfo.userName) return $message.error('請输入用戶名');
      if (!userInfo.password) return $message.error('請输入密码');
      if (!userInfo.verificationCode) {
        return $message.error('請输入验证码');
      }
      loading.value = true;
      http.post('/api/user/login', userInfo, '正在登錄....').then((result) => {
        if (!result.status) {
          loading.value = false;
          getVierificationCode();
          return $message.error(result.message);
        }
        $message.success('登錄成功,正在跳转!');
