...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const open = (item, useRoute) => {
      ...
      if (_index == -1) {
        navigation.push({
          ...
          path: item.path,
          query: item.query, //2021.03.20修复自定义二次打开$tabs時参数丢失的問题
        });
        //新打开的tab移至最后一个选项
    ...
    /**
     * 关闭其它标签页
     * @param {*} par 关闭類型(left,right,other)
     */
    const closeTabs = (value) => {
      ...
      switch (value) {
        ...
        case "other": {
          // 删除其他所有tab标签
          navigation.splice(currnetIndex + 1); // 删除右侧tab标签(這裡必須按照右→左顺序删除)
          navigation.splice(1, currnetIndex - 1); // 删除左侧tab标签
          break;
        }
        default: {
    ...
    const created = () => {
      ...
      http.get("api/menu/getTreeMenu", {}, true).then((data) => {
        data.push({ id: "1", name: "首页", url: "/home" }); // 為了獲取选中id使用
        data.forEach((d) => {
        ...
        //开启消息推送（main.js中設置是否开启signalR）2022.05.05
...
function showTime() {
  ...
  return (
    ...
    "." +
    (day < 10 ? "0" + day : day) + //202.08.08修复日期天数小于10時添加0
    "" +
    " " +
    (hour < 10 ? "0" + hour : hour) +
    ":" +
