let base = {
  ...
  checkUrl(url) {
    // url= 协议://(ftp的登錄信息)[IP|域名](:端口号)(/或?請求参数)
    var strRegex =
      ...
      '((/?)|' + // url无参数結尾 - 斜杆或这没有
      "(/[\\w_!~*'()\\.;?:@&=+$,%#-]+)+/?)$"; // 請求参数結尾- 英文或数字和[]内的各种字符
    var re = new RegExp(strRegex, 'i'); // i不区分大小写
    // 將url做uri转码后再匹配，解除請求参数中的中文和空字符影响
  ...
  // backGroundUrl 后台url，如果后台url直接从后台下载，其他全部通過点击a标签下载
  ...
  // 2020.06.01增加通用方法，將普通对象转换為tree結构
  // data数据格式[
  //     { name: 'tree1', id: 1, parentId: 0 },
  //     { name: 'tree2', id: 2, parentId: 0 }]

  // 1、id與parentId这两个字段必須有
  // 2、树形tree需要注意Id與parentId循环依赖的問题
  // 3、callback每次生成一新的节点的時回調的方法
